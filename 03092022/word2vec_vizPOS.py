	...
	if(useStopWords):
		...
		for sentence in nltk.sent_tokenize(all_text):
			...
			for wrd in nltk.word_tokenize(sentence):
				...
				if wrd.lower() not in stop_words and (myWrdPos[0][1] == 'NN' or myWrdPos[0][1] == 'JJ' or myWrdPos[0][1] == 'VB' or myWrdPos[0][1] == 'VBG' or myWrdPos[0][1] == 'VBD' or myWrdPos[0][1] == 'VBN' or myWrdPos[0][1] == 'VBP'):
...
def getNegationNeighbor(myModel,myWord,myWord2,myWord3,neighborCnt):
	...
	result = myModel.wv.similar_by_vector((vec1 + vec2) - vec3)
...
def plotSimilarityCircleStride(words,scores,myWord,stride,offset):
	...
	for word, score, color in zip(words,scores,colors):
		...
		cnt += 1
	print("SIZES: ",len(words),len(scores),len(colors),len(nscores))
