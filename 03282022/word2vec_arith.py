...
def getAddNeighbor(myModel,myWord,myWord2,neighborCnt):
	vec1, vec2 = myModel.get_vector(myWord), myModel.get_vector(myWord2)
	result = myModel.similar_by_vector(vec1+vec2);
	returnWord = ''
...
def getAllWordEmbeddings(myModel):
	...
	for wrd in list(myModel.vocab):
		embeddings.append(myModel[wrd])
		words.append(wrd)

	return embeddings,words


def analogy(myModel,myWord,myWord2,myWord3,usePreTrained):
	return myModel.most_similar(positive = [myWord,myWord2], negative = [myWord3])[0][0];
...
def main():
	...
	if(len(sys.argv) == 1):
		...
		try:
			opts, args = getopt.getopt(sys.argv[1:], 'f:n:W:w:s:N:i:8TASMDPY')
			for o, a in opts:
				if o == '-f':
					inFile = a
				if o == '-n':
			...
			if(arithMode == "negation"):
				for i in range(0,itr):
					resultWrd = analogy(myModel,myWord,myWord2,myWord3,usePreTrained)
					words, scores = getNeighborSimilarity(myModel,resultWrd,neighborCnt)
					plotSimilarityCircle(arithMode,words,scores,resultWrd,stride,offset,myWord,myWord2,myWord3,i)	
					myWord3 = myWord2
					myWord2 = myWord
